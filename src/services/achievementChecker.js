...
/**
 * Check all badge conditions for a user and unlock any newly earned ones.
 * Returns an array of newly unlocked badges (empty if none).
 *
  ...
  for (const badge of BADGE_DEFINITIONS) {
    ...
    switch (badge.badgeId) {
      ...
      case "streak_30":
        ...
        break;
      case "accuracy_90":
        // Check if any test session had 90%+ accuracy
    ...
    if (earned) {
      try {
        ...
        // Duplicate key error means it was already unlocked (race condition)
        if (err.code !== 11000) {
